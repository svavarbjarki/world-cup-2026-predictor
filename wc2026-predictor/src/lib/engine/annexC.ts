 ...
 *   Annexe C "Combinations for eight best third-placed teams" (pp. 80-97) for
 *   the 495-row table, and Article 12.6 (p. 23) for the round-of-32 match
 *   schedule and the eligible third-place source groups per winner.
 *   PDF: https://digitalhub.fifa.com/m/636f5c9c6f29771f/original/FWC2026_regulations_EN.pdf
 *   Obtained and parsed: 2026-06-07.
 *
 * CONFIDENCE: High. The table below was extracted programmatically from the
 * official PDF (not hand-typed) and passed every consistency check:
 *   - exactly 495 rows, one per C(12,8) combination, all combinations distinct;
 *   - every row has 8 distinct third-place groups;
 *   - no third-place team is ever paired with the winner of its own group;
 *   - every assignment matches the eligible-source-group sets stated
 *     independently in Article 12.6 (3,960 / 3,960 cells consistent).
 *
 * Do not hand-edit individual rows. If a correction is ever needed, re-derive
 * the whole table from the official PDF so the invariants above still hold.
 */

import type { GroupLetter } from "./types";

/**
 * The eight group winners that face a third-placed team, in Annex C column
 * order. Each row of ANNEX_C lists the third-place group assigned to each of
 * these winners, positionally. (Winners C, F, H and J face runners-up instead
 * and so never appear here.)
 */
export const THIRD_PLACE_WINNER_SLOTS = [
  "A",
  "B",
  "D",
  "E",
  ...
  "K",
  "L",
] as const satisfies readonly GroupLetter[];

/**
 * The 495 combinations. Key: the eight qualifying third-place groups, sorted
 * alphabetically and joined (e.g. "ABCDEFGH"). Value: the third-place group
 * assigned to each winner in THIRD_PLACE_WINNER_SLOTS order.
 */
export const ANNEX_C: Readonly<Record<string, readonly GroupLetter[]>> = {
...
/**
 * Look up which third-placed team faces each eligible winner, given the eight
 * groups whose third-placed teams qualified.
 *
 * Returns a Map from winner group letter (one of THIRD_PLACE_WINNER_SLOTS) to
 * the third-place group it faces. Throws if the combination is not exactly
 * eight distinct groups or is not present in Annex C.
 */
export function thirdPlaceOpponentsByWinner(
  qualifyingThirdGroups: readonly GroupLetter[],
): Map<GroupLetter, GroupLetter> {
  const unique = new Set(qualifyingThirdGroups);
  if (unique.size !== 8 || qualifyingThirdGroups.length !== 8) {
    throw new Error(
      `Annex C lookup needs exactly 8 distinct third-place groups, received ` +
        `${qualifyingThirdGroups.length} (${[...unique].sort().join("") || "none"}).`,
    );
  }
  ...
  if (!row) {
    // Should be unreachable for any valid 8-of-12 combination, since all 495
    // are present, but guard anyway rather than return something wrong.
    throw new Error(`No Annex C row for third-place combination "${key}".`);
  }

  const opponents = new Map<GroupLetter, GroupLetter>();
  THIRD_PLACE_WINNER_SLOTS.forEach((winner, index) => {
    opponents.set(winner, row[index]);
  });
  return opponents;
}
